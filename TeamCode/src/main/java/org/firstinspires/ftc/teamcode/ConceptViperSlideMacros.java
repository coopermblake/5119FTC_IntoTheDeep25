...
public class ConceptViperSlideMacros {
    DcMotor slideExt;
    DcMotor slideRot;
    Robot robot;
    ...
    long lastCycle;
    boolean macroRunning;

    public ConceptViperSlideMacros(Robot robot) {
        this.robot = robot;
    ...
    public void moveSlide(double inputRot, double inputExt) {
        if(slideExt.getCurrentPosition()> extMax && inputExt > 0){
            inputExt = 0;
        }
        if(slideExt.getCurrentPosition()< extMin && inputExt < 0){
            inputExt = 0;
        }
    ...
    private void macro1(){
        macroRunning = true;
        slideRot.setTargetPosition(87);
        slideRot.setMode(DcMotor.RunMode.RUN_TO_POSITION);
        slideRot.setPower(0.15);
        macroRunning = false;
    }
}
